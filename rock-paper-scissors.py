...
def show_computer_play(frame, computer_played):

    if computer_played == "Rock":
        image_path = "rock.png"  
    elif computer_played == "Paper":
        image_path = "paper.png"  
    elif computer_played == "Scissors":
        image_path = "scissors.png"  
    else:
        image_path = "unknown_image.png"

    computer_image = cv2.imread(image_path)

    if computer_image is not None:
        scale_percent = 15  
        width = int(computer_image.shape[1] * scale_percent / 100)
        height = int(computer_image.shape[0] * scale_percent / 100)
        dim = (width, height)

        resized_image = cv2.resize(computer_image, dim, interpolation=cv2.INTER_AREA)
        x_offset = 280 
        y_offset = 100  
    ...
    with hands_module.Hands(static_image_mode=False, min_detection_confidence=0.7,
        ...
        while True:
            ...
            if not hold_for_play and not timer_started:
                ...
                show_computer_play(frame, computer_played)
